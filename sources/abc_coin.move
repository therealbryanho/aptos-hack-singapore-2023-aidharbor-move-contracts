module aptos_framework::abc_coin {
    ...
    struct ABCCoin has key {}

    struct MintCapStore has key {
        mint_cap: MintCapability<ABCCoin>,
    }
    ...
    /// Can only called during genesis to initialize the ABC coin.
    public(friend) fun initialize(aptos_framework: &signer): (BurnCapability<ABCCoin>, MintCapability<ABCCoin>) {
        system_addresses::assert_aptos_framework(aptos_framework);

        let (burn_cap, freeze_cap, mint_cap) = coin::initialize_with_parallelizable_supply<ABCCoin>(
            aptos_framework,
            string::utf8(b"ABC Coin"),
            string::utf8(b"ABC"),
            8, // decimals
            true, // monitor_supply
        );
    ...
    public(friend) fun configure_accounts_for_test(
        ...
        // Mint the core resource account ABCCoin for gas so it can execute system transactions.
        coin::register<ABCCoin>(core_resources);
        let coins = coin::mint<ABCCoin>(
            18446744073709551615,
            &mint_cap,
        );
        coin::deposit<ABCCoin>(signer::address_of(core_resources), coins);
    ...
    public entry fun mint(
        ...
        let coins_minted = coin::mint<ABCCoin>(amount, mint_cap);
        coin::deposit<ABCCoin>(dst_addr, coins_minted);
    }
